...
- **Rules engine** — categorize a transaction once, optionally save it as a rule, and future matching transactions are categorized automatically. Rules can match on counterparty name, notes, counter account, own account, amount (exact/greater-than/less-than/between), and direction (debit/credit), combined with AND logic, with priority ordering to resolve conflicts between overlapping rules
